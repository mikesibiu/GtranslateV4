...
echo ""
echo "   Check status (gcloud has no 'translate' subcommand — use curl):"
echo "   TOKEN=\$(gcloud auth print-access-token)"
echo "   curl -s -H \"Authorization: Bearer \$TOKEN\" \\"
echo "     \"https://translation.googleapis.com/v3/projects/${PROJECT}/locations/${LOCATION}/glossaries/ro-en-religious-terms\" \\"
echo "     | python3 -c \"import sys,json; d=json.load(sys.stdin); print(d.get('state','?'), d.get('name','?'))\""
echo "   curl -s -H \"Authorization: Bearer \$TOKEN\" \\"
echo "     \"https://translation.googleapis.com/v3/projects/${PROJECT}/locations/${LOCATION}/glossaries/en-ro-religious-terms\" \\"
echo "     | python3 -c \"import sys,json; d=json.load(sys.stdin); print(d.get('state','?'), d.get('name','?'))\""
echo ""
