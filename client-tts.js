// NovaTranslate TTS methods — speech synthesis, voice cache, queue
...
Object.assign(GTranslateV4Client.prototype, {
    speakTranslation(text, targetLanguage, options = {}) {
        ...
        // Add to queue — cap at 3 items in earbuds mode to prevent audio lag buildup.
        // When translations fire faster than TTS can speak, the queue grows unboundedly
        // and audio falls 20-30s behind live speech. Drop oldest items to stay live.
        const MAX_EARBUDS_QUEUE = 3;
        if (!this.ttsEnabled || this.ttsQueue.length < MAX_EARBUDS_QUEUE) {
            this.ttsQueue.push({ text, targetLanguage });
        } else {
            // Queue full — drop oldest pending item and add new one to stay current
            const dropped = this.ttsQueue.shift();
            console.warn(`⚠️ [TTS] Queue full (>${MAX_EARBUDS_QUEUE}) — dropping oldest to stay live: "${dropped.text.substring(0, 40)}"`);
            this.ttsQueue.push({ text, targetLanguage });
        }
